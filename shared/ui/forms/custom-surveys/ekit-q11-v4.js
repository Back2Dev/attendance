...
const NavigationButtons = ({ page, setPage, isNextDisabled }) => (
...
// --- Main App Component ---
export default function App({ formData, onSubmit, changeModel }) {
  const [page, setPage] = useState(1)
  const [model, setModel] = useState(formData.primary)
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    console.log('Form Submitted:', model, onSubmit)
    changeModel(model)
    onSubmit()
  }
  ...
  const renderPage = () => {
    switch (page) {
      case 1:
        return (
          <FormPage title="Management Strengths & Opportunities (Page 1 of 3)">
            <WrittenResponse
              id="strengths"
              label={`1. What are ${formData.name}'s main leadership/management strengths as a manager?`}
              value={model.strengths}
              onChange={handleChange('strengths')}
              placeholder="Describe your key strengths..."
            />
            <RadioResponseTable
              question={`2. Every leader/manager has opportunities for growth and improvement. What would you consider to be ${formData.name}'s Primary Function of Management Opportunity?`}
      ...
      case 2:
        ...
        return (
          <FormPage title="Management Strengths & Opportunities (Page 2 of 3)">
            <RadioResponseTable
              question={`4. What would you consider to be ${formData.name}'s SECONDARY Function of Management Opportunity?`}
      ...
      case 3:
        return (
          <FormPage title="Management Strengths & Opportunities (Page 3 of 3)">
            <WrittenResponse
