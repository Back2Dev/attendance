...
export default class Q3Bridge extends Bridge {
  schema: SimpleSchema;
  ...
  getErrorMessages(error: any) {
    ...
    return Array.isArray(details)
      ? // @ts-expect-error: `messageForError` has incorrect typing.
        details
          // .filter((err) => !err.name.match(/^activities\.\d+/))
          .map((error) => this.schema.messageForError(error))
      : [error.message || error];
  }
  ...
  getProps(name: string) {
    ...
    if (typeof props.uniforms === 'function' || typeof props.uniforms === 'string') {
