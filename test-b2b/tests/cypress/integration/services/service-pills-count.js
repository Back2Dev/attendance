...
describe('log into app, create a job and test the service pills', () => {
  before(function () {
    freshDatabase()
  })
  ...
  it('creates another job and searches for the customer', () => {
    ...
    // opens manager menu navigates to create service page and adds minor service', function () {
    cy.get('[data-cy=adm-drawer]').click()
    cy.get('div').contains('Manager').click({ force: true })
    cy.get('[href="/services/new"]').should('exist').click()
    cy.get('#service-total').should('exist')
    cy.get('[data-cy=minor]').click()
    cy.get('div').contains('Bar tape - $20').click()
    ...
    cy.get('[name="email"').clear().type('vernon.wiza@testa.rossa')
  ...
  it('searches for a job and checks if the pill contains number of jobs searched for', () => {
    cy.visit('/services')
    cy.get('[data-cy=email-input]').type('mike.king@mydomain.com.au')
    cy.get('[data-cy=password-input]').type('me2')
    cy.get('[data-cy=login-btn]').should('exist').click()
    ...
    cy.wait(100)
    cy.get('div').contains('Giganto').should('exist')
    cy.get('[placeholder="Search ..."]').clear().type('Pat Carmel').type('{enter}')
    cy.get('[tabindex="0"]').should('contains, New (1)').should('exist')
  })
})
