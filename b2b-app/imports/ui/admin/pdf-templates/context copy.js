...
const PdfTemplateContext = React.createContext('pdf-template')
...
export const PdfTemplateProvider = (props) => {
  ...
  const { loadingPdfs, items } = useTracker(() => {
    try {
      ...
      const pdfs = PdfTemplates.find({}).map((row) => {
        row.search = obj2Search(row)
        console.log('row ', row)
        return row
      })
      return {
        loadingPdfs: !subHandleItems.ready(),
        items: pdfs,
      }
    } catch (e) {
      console.log('error: ', e)
  ...
  const { loadingPdf, item } = useTracker(() => {
    ...
    return {
      loadingItems: !subHandleEdit.ready(),
      item: PdfTemplates.findOne(id) || {},
    }
  }, [id])
  ...
  return (
    <PdfTemplateContext.Provider value={{ ...pdfTemplates }}>
      {children}
