...
const insert = (form) => meteorCall('insert.sessions', 'adding', form)
const add = () => push(`/admin/sessions/add`)
const edit = (id) => push(`/admin/sessions/edit/${id}`)
const view = (id) => push(`/admin/sessions/view/${id}`)
const archive = async (rowids) => {
  const name = prompt('Please enter a name for the archive')
  const text = confirm(
    `Are you sure you want to archive this Sessions and related entities?`
  )

  if (name && text) {
    meteorCall('archive.sessions', `Archiving Sessions to ${name}`, {
      name,
      ids: rowids,
    })
  }
...
const SessionsWrapper = (props) => {
  push = useHistory()?.push
  if (props.loading) return <div>Loading...</div>
  return <SessionsList {...props}></SessionsList>
}

const SessionsLister = withTracker((props) => {
  const subsHandle = Meteor.subscribe('all.sessions')
  const items = Sessions.find({}).map((row) => {
