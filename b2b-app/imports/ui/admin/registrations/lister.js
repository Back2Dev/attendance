...
const update = (form) => meteorCall('update.registrations', 'updating', form)
const insert = (form) => meteorCall('insert.registrations', 'adding', form)
const add = () => push(`/admin/registrations/add`)
const edit = (id) => push(`/admin/registrations/edit/${id}`)
const view = (id) => push(`/admin/registrations/view/${id}`)
const archive = async (rowids) => {
  const name = prompt('Please enter a name for the archive')
  const text = confirm(
    `Are you sure you want to archive this Registrations and related entities?`
  )

  if (name && text) {
    meteorCall('archive.registrations', `Archiving Registrations to ${name}`, {
      name,
      ids: rowids,
    })
  }
