...
const remove = (id) => meteorCall('rm.rentals', 'Deleting', id)
const update = (form) => meteorCall('update.rentals', 'updating', form)
const insert = (form) => meteorCall('insert.rentals', 'adding', form)
const add = () => push(`/admin/rentals/add`)
const edit = (id) => push(`/admin/rentals/edit/${id}`)
const view = (id) => push(`/admin/rentals/view/${id}`)
const archive = async (rowids) => {
  const name = prompt('Please enter a name for the archive')
  const text = confirm(
    `Are you sure you want to archive this Rentals and related entities?`
  )

  if (name && text) {
    meteorCall('archive.tools', `Archiving Rentals to ${name}`, {
      name,
      ids: rowids,
    })
  }
...
const RentalsLister = (props) => {
  push = useHistory()?.push
  const { items, loading } = useTracker(() => {
    const subsHandle = Meteor.subscribe('all.tools')
    const items = Rentals.find({}).map((row) => {
  ...
  return <ToolsList {...props} items={items} methods={methods} columns={columns} />
}

export default RentalsLister
