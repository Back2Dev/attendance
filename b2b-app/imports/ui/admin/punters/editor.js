...
const update = (id, form) => {
  meteorCall('update.punters', 'updating', form)
  history.push('/admin/punters')
}
...
const Editor = withTracker((props) => {
  ...
  const subsHandle = Meteor.subscribe('id.punters', id)
  const item = Punters.findOne(id) || {}
  return {
    id,
    item,
    methods,
...
//       const response = await meteorCall('fetch.id.punters', null, id)
