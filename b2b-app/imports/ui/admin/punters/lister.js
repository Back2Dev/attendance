...
const remove = (id) => meteorCall('rm.punters', 'Deleting', id)
const update = (form) => meteorCall('update.punters', 'updating', form)
const insert = (form) => meteorCall('insert.punters', 'adding', form)
const add = () => push(`/admin/punters/add`)
const edit = (id) => push(`/admin/punters/edit/${id}`)
const view = (id) => push(`/admin/punters/view/${id}`)
const archive = async (rowids) => {
  const name = prompt('Please enter a name for the archive')
  const text = confirm(
    `Are you sure you want to archive this Punters and related entities?`
  )

  if (name && text) {
    meteorCall('archive.punters', `Archiving Punters to ${name}`, {
      name,
      ids: rowids,
    })
  }
...
const PuntersWrapper = (props) => {
  push = useHistory()?.push
  if (props.loading) return <Loader loading />
  return <PuntersList {...props}></PuntersList>
}

const PuntersLister = withTracker((props) => {
  const subsHandle = Meteor.subscribe('all.punters')
  const items = Punters.find({}).map((row) => {
