...
export const masterTicker = async () => {
  try {
    let n = 0
    const readyJobs = await Cronjobs.find({
      active: true,
      status: 'ready',
      $or: [{ nextRun: { $lte: new Date() } }, { nextRun: null }],
    }).fetchAsync()
    for (const cronjob of readyJobs) {
      // Set it to running to avoid double-ups
      await Cronjobs.updateAsync(cronjob._id, {
    ...
    const runningJobs = await Cronjobs.find({
      active: true,
      status: 'running',
    }).fetchAsync()
    for (const cronjob of runningJobs) {
