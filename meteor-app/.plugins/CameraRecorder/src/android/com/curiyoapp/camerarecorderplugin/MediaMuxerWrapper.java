...
public class MediaMuxerWrapper {
	 ...
	 * @throws IOException
	 */
	public MediaMuxerWrapper(MuxerListener listener, String path, long maxDurationUs) throws IOException {
		this.listener = listener;
		this.maxDuration = maxDurationUs;
...
//**********************************************************************
	...
	private final MuxerListener listener;

	private final long maxDuration;

	private final Object timeLock = new Object();
	private long currentMaxTime = 0;
	private boolean stopAcceptingNewFrames = false;

	public final boolean reportNewTime(MediaEncoder encoder, long frameTime) {
		synchronized (timeLock) {
			if (frameTime > maxDuration) {
				stopAcceptingNewFrames = true;
				return false;
			}

			if (stopAcceptingNewFrames) {
				return frameTime <= currentMaxTime;
			} else {
				if (frameTime > currentMaxTime) {
					currentMaxTime = frameTime;
					listener.onTimestamp(this, currentMaxTime);
				}
