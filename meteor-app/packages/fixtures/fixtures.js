      ...
      if (
        ...
        id = Accounts.createUser({
          ...
          password: user.password
            ? user.password
            : Fixtures.config.defaultPassword,
        ...
        if (Fixtures.config.debug)
          console.log('User exists ' + user.email, user.username)
      }
    } catch (e) {
...
Fixtures.loadThese = function (list, which) {
  ...
  if (!(Meteor.settings.env && Meteor.settings.env.environment))
    console.warn(
      'No env.environment in your settings file, defaulting to ' + env
    )
  ...
  Fixtures.config[list]
    ...
    .forEach((thing) => {
      if (!thing.name || !thing.key || !thing.table)
        ...
        if (thing.name === which) {
          if (thing.name.match(/\-test/i))
            collection.remove({ [thing.key]: /^test/ })
        ...
        if (thing.env) {
          ...
          if (!environs.includes('all') && !environs.includes(env))
            doit = false
        }
        if (!doit) {
          console.log(
            'Not loading ' + thing.name + ' in environment ' + env
          )
        } else {
          if (Fixtures.config.debug)
            console.warn(' Checking ' + thing.name)
          const rowData = Fixtures.data[thing.name] || []
          if (!rowData.length)
            console.warn(`No fixtures data found for ${thing.name}`)
          rowData.forEach((c) => {
            ...
            if (collection.find(criteria).fetch().length === 0) {
              if (Fixtures.config.debug)
                console.log(
                  "  Adding '" + c[thing.key] + "' to " + thing.name
                )
              ...
              if (thing.related) {
                let criteria = {}
                criteria[thing.related.lookup] =
                  c[thing.related.reference]
                let rec = thing.related.table.findOne(criteria)
                if (Fixtures.config.debug)
                  console.log('Looking up fk', criteria, rec)
                if (rec) {
              ...
              if (thing.user) {
                ...
                if (!urec) {
                  let uid = Accounts.createUser({
                    ...
                    password: c.password
                      ? c.password
                      : thing.user.defaultPass,
                    username: c.name,
                    profile: { name: c.name },
                  })
                  c[thing.user.id] =
                    typeof thing.user.id === 'object' ? [uid] : uid
                } else {
                  if (Fixtures.config.debug)
                    console.log(
                      thing.name + ' user exists: ' + c.name
                    )
                  c[thing.user.id] =
                    typeof thing.user.id === 'object'
                      ? [urec._id]
                      : urec._id
                }
              }
              if (Fixtures.config.debug)
                debug(`Inserting ${c[thing.key]}`)
              try {
                collection.insert(c)
              } catch (e) {
                console.error(
                  `Error loading fixture ${thing}: ${e.message}`
                )
              }
            } else {
              if (Fixtures.config.debug)
                console.log(" '" + c[thing.key] + "'  exists")
            }
          })
        }
      }
