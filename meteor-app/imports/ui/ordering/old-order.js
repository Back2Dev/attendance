    ...
    <div>
      <Container textAlign="center">
        <Segment raised clearing className="review">
          <Header as="h3">{humaniseDateDay(order.updatedAt)}</Header>
            ...
            <Table.Body>
              {!props.loading &&
                ...
                order.orderedParts.map(part => (
                  <Component className="cart-item" key={part.partId} {...part} order={order} oldOrder={true} />
                ))}
            </Table.Body>
          </Table>
          <Input fluid label="Additional Notes" />

          {!props.loading && order && <Header as="h2">Total Price: ${(order.totalPrice / 100).toFixed(2)}</Header>}
          <br />
        </Segment>
      </Container>
    </div>
