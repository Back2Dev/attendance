...
describe('Shopping Payment', function () {
  ...
  it('Confirm Cart', function () {
    cy.get('button').contains('Buy now').should('exist').click()
  })
  it('Add Personal Details', function () {
    cy.get('h2')
      .contains('Payment form - your billing address')
      .should('exist')
    cy.get('input[name="email"]').clear().type('manhhuyvo@gmail.com')
    ...
    cy.get('input[name="address_city"]')
      .clear()
      .type('Port Melbourne')
    cy.get('input[name="address_state"]').clear().type('VICTORIA')
    cy.get('input[name="address_postcode"]').clear().type('3000')
    cy.get('input[name="address_country"]').clear().type('AUSTRALIA')
    ...
    Cypress.Commands.add('pinType', (field, text) => {
      cy.get('#' + field).within(() => {
        cy.get(`iframe`).then(($iframe) => {
          ...
          cy.wrap(body).find(`.${field}`).type(text)
        })
      })
    })
