...
const required = 'from, to, tag,pipeline'.split(/[\s,;]+/)

if (opts.help || !required.every((option) => opts[option])) {
  console.log(`
#Usage:
	node scripts/flowlu-import-opportunities.js --from=nnn --to=nnn --pipeline=nnn --tag=xxx
Where
  --from=nnn - number of "from" stage
  --to=nnn  - number of "to" stage
  --pipeline=nnn  - id of pipeline
...
const doit = async () => {
  try {
    ...
    while (!done) {
      ...
      contacts.data.response.items.forEach((contact) => {
        if (
          ...
          contact.pipeline_stage_id === from &&
          contact.pipeline_id === pipeline
        )
    ...
    for (person of persons) {
      ...
      debug('updating opportunities  ', url)
      let params = querystring.stringify({
        pipeline_stage_id: to,
      })
